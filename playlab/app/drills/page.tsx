...
import { StatusPill, DrillTag, EngagementMechanicPill } from "@/components/ui";
import { AddToSessionButton } from "@/components/drills/AddToSessionButton";
  ...
  return (
    <div className="mx-auto max-w-5xl">
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <Link href={`/drills/new${sessionId ? `?sessionId=${sessionId}` : ""}`} className="btn-primary">
        ...
        {drills.map((d) => (
          <div key={d.id} className="glass flex flex-col p-5">
            <div className="flex items-start justify-between gap-2">
              <Link href={`/drills/${d.id}`} className="text-lg font-semibold text-cream hover:text-blue">
                {d.name}
              </Link>
            ...
            <div className="mt-4 flex gap-2 border-t border-line pt-3">
              <Link href={`/drills/${d.id}`} className="btn-ghost !px-3 !py-1.5 !text-[12px]">
                Details
              </Link>
              <Link href={`/drills/${d.id}/edit${sessionId ? `?sessionId=${sessionId}` : ""}`} className="btn-ghost !px-3 !py-1.5 !text-[12px]">
                Rule studio
              </Link>
              {sessionId && (
