...
import { useDrillBuilder } from "@/playlab/store/drill-builder-store";
import { generateDrillConcepts } from "@/playlab/lib/drill-generator";
import { analyseSession, dimensionLabel } from "@/playlab/lib/session-analysis";
import { saveConceptAction } from "@/playlab/lib/actions";
import { FlowBadge } from "@/playlab/components/ui";
import type {
  CompetitionFormat,
  DrillView,
  EngagementMechanic,
...
function Num({ label, k, min, max }: { label: string; k: never | keyof import("@/playlab/types").DrillConstraints; min: number; max: number }) {
  const b = useDrillBuilder();
  return (
...
function Sel({ label, k, options }: { label: string; k: keyof import("@/playlab/types").DrillConstraints; options: string[] }) {
  const b = useDrillBuilder();
  return (
    <label className="block">
      <span className="label-sm">{label}</span>
